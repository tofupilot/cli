...
impl IdentifyHost for CliIdentifyHost {
    ...
    async fn prompt(
        ...
        // Optional agent timeout. Unlike a regular `ui_request` we
        // don't fall back to an empty response: identify-unit is a
        // hard dependency for the run, so timing out is a clean
        // cancellation that the run loop turns into a `RunCrashed`.
        let timeout = self.agent.as_ref().and_then(|a| a.ui_timeout);
        let result = match timeout {
            Some(t) => match tokio::time::timeout(t, resp_rx).await {
            ...
            None => match resp_rx.await {
                Ok(values) => Ok(values),
                Err(_) => Err(IdentifyHostError::Cancelled(
                    "operator UI channel closed before responding".to_string(),
                )),
            },
        };
